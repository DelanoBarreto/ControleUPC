...
import type { CampoModeloTemplate } from "@/lib/modelos/templates";
...
type AnexoItem = {
  ...
  template_campo_id?: string | null;
  signed_url_original: string | null;
  signed_url_pdf: string | null;
  created_at: string;
};

export type ValorCampoTemplate = {
...
function buildInitialDrafts(campos: CampoModeloTemplate[], valores: ValorCampoTemplate[]) {
  const mapa = new Map(valores.map((valor) => [valor.template_campo_id, valor]));
...
function groupCamposBySecao(campos: CampoModeloTemplate[]) {
  const grupos = new Map<string, CampoModeloTemplate[]>();
  const ordem: string[] = [];
...
async function fetchValoresTemplate(templateCodigo: string, prestacaoContasId: string) {
  const response = await fetch(
    `/api/modelos/templates/${encodeURIComponent(templateCodigo)}?prestacao_contas_id=${encodeURIComponent(prestacaoContasId)}`
  );
  ...
  const data = (await response.json()) as {
    campos: Array<CampoModeloTemplate & { valor: ValorCampoTemplate | null }>;
  };
  return data.campos.map((campo) => campo.valor).filter((valor): valor is ValorCampoTemplate => Boolean(valor));
}

async function fetchAnexosTemplate(templateCodigo: string, prestacaoContasId: string) {
  const response = await fetch(
    `/api/anexos/templates/${encodeURIComponent(templateCodigo)}?prestacao_contas_id=${encodeURIComponent(prestacaoContasId)}`
  );
...
export function TemplateEditorClient({ campos, contexto, templateCodigo, titulo, prestacaoContasIdInicial, valores }: Props) {
    ...
    void (async () => {
      try {
        const [valoresCarregados, anexosCarregados] = await Promise.all([
          fetchValoresTemplate(templateCodigo, prestacaoContasIdInicial),
          fetchAnexosTemplate(templateCodigo, prestacaoContasIdInicial)
        ]);
  ...
  }, [campos, prestacaoContasIdInicial, templateCodigo]);
  ...
  async function carregarValores(prestacaoId?: string) {
    ...
    try {
      const [valoresCarregados, anexosCarregados] = await Promise.all([
        fetchValoresTemplate(templateCodigo, id),
        fetchAnexosTemplate(templateCodigo, id)
      ]);
      setDrafts(buildInitialDrafts(campos, valoresCarregados));
      setAnexos(anexosCarregados);
      setStatus("Valores carregados.");
  ...
  async function carregarAnexos(prestacaoId?: string) {
    ...
    try {
      setAnexos(await fetchAnexosTemplate(templateCodigo, id));
    } catch {
  ...
  async function salvarCampo(campo: CampoModeloTemplate) {
    ...
    try {
      const response = await fetch("/api/modelos/templates/valores", {
        method: "POST",
  ...
  async function enviarAnexo() {
    ...
    formData.set("template_campo_id", anexoCampoId);
    ...
    try {
      const response = await fetch(`/api/anexos/templates/${encodeURIComponent(templateCodigo)}`, {
        method: "POST",
        body: formData
      });
            ...
            {anexos.length === 0 ? (
              ...
              anexos.map((anexo) => (
                <div className="rounded-[18px] border border-slate-200 bg-white px-3 py-3" key={anexo.id}>
                  ...
                  <p className="mt-1 text-xs text-[var(--muted)]">
                    Campo: {anexo.template_campo_id ?? anexo.campo_modelo_id ?? "sem vínculo"}
                  </p>
                </div>
              ))
            )}
